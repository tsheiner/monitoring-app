...
The corrected view makes the distribution footprint, slopes, and ridge visible before inspecting the trace. The measured trace receives a dark contact treatment where historical density is meaningful and loses it outside the terrain.
...
The ridge is immediately distinguishable from the slopes. The trace begins above the terrain, crosses the outer boundary, passes through the ridge, and later leaves below it. The contact treatment makes these transitions legible without a tooltip.
...
The palette and contour logic remain effective at a very different metric scale. Broad and narrow terrain sections remain distinguishable, and the trace contact treatment follows the same density semantics.
...
| --- | --- | --- |
| Is the measured value typical? | Pass | The warm ridge and density-responsive contact make trace proximity to the ridge visible in all three metrics. |
| Where does it become unusual? | Pass | The outer contour and fully transparent exterior create a decisive transition; contact disappears as the trace leaves support. |
...
| Is the measured series still primary? | Pass | The original core path remains unchanged and renders above the contact and terrain layers. |

## Interaction and restoration findings

- Bands/Terrain switching preserves chart state and creates no duplicate layers.
- Terrain contact is absent in Bands and multi-metric modes.
- Crosshair, tooltip, event, zoom, range, resize, and live-data tests pass with the new layers.
- Disabling and re-enabling a metric exposed a stale baseline-loaded flag. The flag is now invalidated when the metric is removed, and live browser verification confirms Terrain and trace contact return after re-enable.
...
The existing preview path is active and covered at device-pixel ratio 2: a 920 CSS-pixel-wide plot uses a 460-pixel preview backing width during interaction and restores an 1840-pixel backing width on release. Full-resolution rendering remains the settled result after interaction.
